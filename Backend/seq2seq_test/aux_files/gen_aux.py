...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 29 19:04:28 2020
...
def create_static_conditions(allChords, allDurs, allMels):
    #remove bar and eos elements
    allDurs = allDurs[:-1]
    allMels = allMels[:-1]    
    allChords = allChords[:-1]
...
def generate_chord_durs_ev_seq(encoder_chords_ev, decoder_chords_ev, ChordEncoders_ev, val_templates_ev, timesig, 
                             temperature, numOfBars, valence):
    
    #create the encoder list
    enc_list = create_encoder_ev(ChordEncoders_ev,timesig,numOfBars,val_templates_ev,valence)
    
    enc_inp = np.array(enc_list).reshape(1,-1)
    
    dec_seq_length = 358
    #enc_seq_length = 291
    
    dec_sos_idx = int(np.where(ChordEncoders_ev[1].categories_[0] == 'sos')[0])+1
    dec_eos_idx = int(np.where(ChordEncoders_ev[1].categories_[0] == 'eos')[0]) 
    
    dec_out = generate_chordur_ev_seq(encoder_chords_ev, decoder_chords_ev, enc_inp, temperature,
                      dec_sos_idx, dec_eos_idx, dec_seq_length)
    
    
    allChords, allDurs, allMels = convert_to_ChordDurs(dec_out,ChordEncoders_ev)       
    
    return allChords, allDurs, allMels


def create_encoder_ev(TransEncoders,timesig,numOfBars,val_templates,valence):
    ...
    lgt = str(numOfBars)   
    enc_val = deepcopy(choice(list(val_templates[lgt][valence]))) 
    ...
    for i in range(0,numOfBars):
        ...
        enc_list.extend([enc_timesig_idx,curr_group_idx,curr_val_idx,enc_bar_idx ])
...
def generate_chordur_ev_seq(encoder_chords_ev, decoder_chords_ev, enc_inp, temperature,
                      dec_sos_idx, dec_eos_idx, dec_seq_length):
  # Encode the input as state vectors.

  enc1_h, enc1_c, enc2_h, enc2_c, enc3_h, enc3_c  = encoder_chords_ev.predict([enc_inp])
  ...
  while not stop_condition:
      #call the decoder
      dec_out, dec1_h, dec1_c, dec2_h, dec2_c, dec3_h, dec3_c = decoder_chords_ev.predict(
      ...
      if dec_pred == dec_eos_idx or len(decoded_out) >= dec_seq_length:
         stop_condition = True
         print('Generated',len(decoded_out)+1,'steps')
         cnt = 0
...
def convert_to_ChordDurs(dec_out,TransEncoders):
...
def chords_mel_mid(f_chords,f_durs,f_bars,allMelody,timesig,prefix):
    ...
    midi_out = './Backend/seq2seq_test/generations/'
    
    #create the m21 instance
    ...
    for m in range(0,len(all_bars)-1):
        ...
        for b in range(0,len(bar_chords)):
            ...
            mel = bar_melody[b]
            if cho == 'Rest':
               aChord = m21.note.Rest() 
               aChord_name = 'Rest'
            else:
               aChord = m21.harmony.ChordSymbol(cho)
               pitchNames = [p for p in aChord.pitches]
               aChord = m21.chord.Chord(pitchNames)
               aChord_name = aChord.pitchedCommonName
            if prevChord == aChord_name:
                apC = cho_m.pop(-1)
                apC.quarterLength += float(dur)
                cho_m.append(apC)
            else:
                aChord.offset = offset
                aChord.quarterLength = float(dur)
                cho_m.append(aChord)
        ...
        chords.append(cho_m)
        melody.append(mel_m)
    #append the parts to the stream
    rc.append(melody)
    rc.append(chords)
